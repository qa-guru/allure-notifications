...
Collage PNG + текст со статистикой и ссылками. Соберите `config.json` в [Config builder](#config-builder-anb), отправьте через CLI на TypeScript **6.2.2**.
...
| **6.\*** | TypeScript | Allure 3 | **Продукт** — pin **6.2.2** (CLI + builder + plugin) |

Патч-ноты → [GitHub Releases](https://github.com/qa-guru/allure-notifications/releases) · миграция → [`MIGRATION.md`](MIGRATION.md).

| Часть | Роль |
|-------|------|
| **CLI** | `npx @qa-guru/allure-notifications@6.2.2 send --config …` — основной runtime |
...
Top-level `proxy` (`type`: `http` \| `socks5`, `host`, `port`, опционально `username` / `password`) — CLI live Telegram через undici `ProxyAgent` / `Socks5ProxyAgent` (jar: Apache HttpClient). При заданном proxy `proxychains4` для Bot API не нужен. SOCKS с auth (qa.guru `proxy.qaguru.school:7777`) требует `username`/`password` в config — на агентах Jenkins сначала `/opt/qa-guru/bin/prepare-telegram-socks-proxy.sh` (подставляет microsocks creds).
