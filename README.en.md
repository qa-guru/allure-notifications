...
+ qa.guru Jenkins SOCKS (`proxy.qaguru.school:7777`): `"proxy": { "type": "socks5", "host": "proxy.qaguru.school", "port": 7777 }` in `config.json` (credentials come from the agent). wget [4.11.1](https://github.com/qa-guru/allure-notifications/releases/download/4.11.1/allure-notifications-4.11.1.jar) — not 4.6.1 / 4.11.0.
+ Configuration can be overridden via system properties (The system property will take precedence if the same configuration
  item is specified in the configuration file). 
  ```shell
