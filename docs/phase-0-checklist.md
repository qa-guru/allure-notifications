...
Hub clone `allure-notifications-builder/` may remain as a mirror until archive (row 4); stand cwd is the monorepo path above. Row **5** domain cutover done — prod [`allure-notifications.qa.guru`](https://allure-notifications.qa.guru/) on this repo. Row **4** archive stays `[ ]` until separate HQ OK ([`pages-cutover.md`](pages-cutover.md) §C).
