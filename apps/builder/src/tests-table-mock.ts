...
export type TestsTableMockStatus = 'passed' | 'failed' | 'broken' | 'skipped';

export type TestsTableMockRow = {
  name: string;
  paletteName: string;
  title?: string;
  status: TestsTableMockStatus;
  trendPoints: string;
  trendStroke?: string;
  flakyFlips: number;
  stability: TestsTableMockStatus[];
};

export const TESTS_TABLE_MOCK_ROWS: readonly TestsTableMockRow[] = [
  {
    name: 'shouldLogin…',
    paletteName: 'login',
...
function stabilityDotsHtml(statuses: readonly string[], limit?: number) {
...
function stabilityCellHtml(flakyFlips: number, statuses: readonly string[], palette = false) {
  const badge =
    flakyFlips > 0 && !palette
...
export function renderTestsTableCanvasRow(
  row: TestsTableMockRow,
...
export function renderTestsTablePaletteRow(row: TestsTableMockRow) {
  return (
    `<tr>` +
...
export function canvasTestsTableRowsHtml(count = TESTS_TABLE_MOCK_ROWS.length) {
  ...
  for (let i = 0; i < n; i += 1) {
    ...
    parts.push(
      renderTestsTableCanvasRow(base, {
