...
const DURATION_BINS: { label: string; segs: { n: number; fill: string }[] }[] = [
  {
    label: '0–6s',
    segs: [
...
function isKitProfile(): boolean {
  const chart = /** @type {{ profile?: string }} */ (state.base.chart);
  return normalizeChartProfile(chart.profile) === 'kit';
}

function tileTier(host: HTMLElement): string {
  const tile = host.closest('.widget-tile');
  if (!tile) return 'regular';
  if (tile.classList.contains('widget-tile--tier-hero')) return 'hero';
  if (tile.classList.contains('widget-tile--tier-compact')) return 'compact';
  if (tile.classList.contains('widget-tile--tier-micro')) return 'micro';
  if (tile.classList.contains('widget-tile--tier-regular')) return 'regular';
  return 'regular';
}

function plotBox(host: HTMLElement): { W: number; H: number } {
  const base = 240;
  ...
  const aw =
    body.clientWidth - (parseFloat(cs.paddingLeft) || 0) - (parseFloat(cs.paddingRight) || 0);
  const ah =
    body.clientHeight - (parseFloat(cs.paddingTop) || 0) - (parseFloat(cs.paddingBottom) || 0);
  if (aw < 24 || ah < 24) return { W: base, H: base };
  let aspect = aw / ah;
  if (aspect > 4) aspect = 4;
  else if (aspect < 0.25) aspect = 0.25;
  if (aspect >= 1) return { W: Math.round(base * aspect), H: base };
  return { W: base, H: Math.round(base / aspect) };
}

function svgOpen(label: string, kind: string, W: number, H: number): string {
  return (
...
function vBarPath(x: number, y: number, w: number, h: number, rTop: number, rBot: number): string {
  ...
  if (rt > 0) d += `A${rt.toFixed(1)},${rt.toFixed(1)} 0 0 1 ${x2.toFixed(1)},${(y + rt).toFixed(1)}`;
  d += `L${x2.toFixed(1)},${(y2 - rb).toFixed(1)}`;
  if (rb > 0) d += `A${rb.toFixed(1)},${rb.toFixed(1)} 0 0 1 ${(x2 - rb).toFixed(1)},${y2.toFixed(1)}`;
  d += `L${(x + rb).toFixed(1)},${y2.toFixed(1)}`;
  if (rb > 0) d += `A${rb.toFixed(1)},${rb.toFixed(1)} 0 0 1 ${x.toFixed(1)},${(y2 - rb).toFixed(1)}`;
  d += `L${x.toFixed(1)},${(y + rt).toFixed(1)}`;
  if (rt > 0) d += `A${rt.toFixed(1)},${rt.toFixed(1)} 0 0 1 ${(x + rt).toFixed(1)},${y.toFixed(1)}`;
  return `${d}Z`;
}

function gridY(x0: number, x1: number, y: number): string {
  return (
    `<line x1="${x0.toFixed(1)}" y1="${y.toFixed(1)}" x2="${x1.toFixed(1)}" y2="${y.toFixed(1)}" ` +
...
export function stockPyramidSvg(host: HTMLElement): string {
  ...
  FUNNEL_LAYERS.forEach((layer, i) => {
    ...
    const y2 = y + bandH;
    parts.push(
      `<path d="M${x0.toFixed(1)} ${y.toFixed(1)} L${x1.toFixed(1)} ${y.toFixed(1)} ` +
    ...
    if (showLegend) {
      ...
      parts.push(
        `<text x="${padX}" y="${ly.toFixed(1)}" font-family="var(--font-sans)" font-size="${fs}" ` +
          `font-weight="600" fill="var(--color-text)">Layer: ${layer.k}</text>`,
        `<text x="${padX}" y="${(ly + fs * 1.15).toFixed(1)}" font-family="var(--font-sans)" ` +
          `font-size="${Math.max(6, fs - 1)}" fill="var(--color-text-muted)">` +
          `Number of tests: ${layer.n} (${layer.pct})</text>`,
        `<text x="${padX}" y="${(ly + fs * 2.25).toFixed(1)}" font-family="var(--font-sans)" ` +
          `font-size="${Math.max(6, fs - 1)}" fill="var(--color-text-muted)">` +
...
export function stockDurationDynamicsSvg(host: HTMLElement): string {
  const tier = tileTier(host);
  ...
  DYNAMICS_BARS.forEach((bar, i) => {
    ...
    if (showAxis) {
      parts.push(
        `<text x="${(x + barW / 2).toFixed(1)}" y="${(H - 8).toFixed(1)}" text-anchor="middle" ` +
  ...
  if (showAxis) {
    parts.push(
      `<text x="${padL - 4}" y="${(padT + 4).toFixed(1)}" text-anchor="end" ` +
        `font-family="var(--font-sans)" font-size="8" fill="var(--color-text-muted)">2m</text>`,
      `<text x="${padL - 4}" y="${(padT + plotH).toFixed(1)}" text-anchor="end" ` +
...
export function stockDurationsHistogramSvg(host: HTMLElement): string {
  const tier = tileTier(host);
  const { W, H } = plotBox(host);
  const showAxis = tier === 'hero' || tier === 'regular';
  const padT = 8;
  ...
  DURATION_BINS.forEach((bin, i) => {
    ...
    bin.segs.forEach((seg, si) => {
      ...
      if (d) parts.push(`<path d="${d}" fill="${seg.fill}" />`);
    });
    if (showAxis) {
      parts.push(
        `<text x="${(x + barW / 2).toFixed(1)}" y="${(H - 8).toFixed(1)}" text-anchor="middle" ` +
...
function paintStockTile(tile: Element): void {
...
export function applyProfileMocks(scope?: ParentNode | null): void {
  if (!scope || isKitProfile()) return;
