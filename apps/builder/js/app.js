...
const TG_LINK_KEYS = Object.freeze(['report', 'dashboard', 'testops', 'build']);
 ...
 * @param {string} raw
 */
function normalizeVectorId(raw) {
 ...
 * @param {Record<string, unknown>} snap
 */
function fingerprintFromSnap(snap) {
...
function rememberSnap(snap) {
  ...
  try {
    const obj = /** @type {Record<string, Record<string, unknown>>} */ ({});
 ...
 * @param {AnbChartItem} item
 */
function freeCellRect(chart, item) {
  const cardGap =
 ...
 */
function canvasDisplayHeight(canvas) {
  const chart = /** @type {{ width: number, height: number }} */ (state.base.chart);
...
function applySnap(snap) {
  ...
  hydrateControls();
  applyCanvasMetrics();
  const chart = /** @type {{ items?: AnbChartItem[] }} */ (state.base.chart);
...
function phrasesFor(lang) {
  return PHRASES[/** @type {keyof typeof PHRASES} */ (lang)] || PHRASES.en;
}
 ...
 * @param {number} total
 */
function formatPercentage(input, total) {
...
function renderCollageStage(stage, mode) {
  const chart = /** @type {{ width: number, height: number, items: AnbChartItem[] }} */ (
    state.base.chart
  );
...
/**
 * @param {AnbChartItem} a
 * @param {AnbChartItem} b
 */
...
/** @returns {AnbChartItem[]} */
function readItemsFromGrid() {
  if (!grid) return [];
  return grid.engine.nodes
    ...
    .filter(/** @returns {item is AnbChartItem} */ (item) => item != null);
}

function syncItemsToState() {
  if (suppressSync) return;
  const chart = /** @type {{ layout: string, items: AnbChartItem[], gridCols: number, gridRows: number }} */ (
    state.base.chart
  );
  chart.layout = 'free';
 ...
 */
function findFreeSpot(w, h, preferX, preferY) {
...
function wireEditorChrome() {
  ...
  if (gridEl) {
    ...
    function onPanelAction(e) {
      ...
      const item = /** @type {HTMLElement | null} */ (actionBtn.closest('.grid-stack-item'));
      if (!item) return;
    ...
    gridEl.addEventListener('click', (e) => {
      ...
      const hit = /** @type {HTMLElement | null} */ (target.closest('.grid-stack-item'));
      if (hit && gridEl.contains(hit)) selectItem(hit);
    });
  }
...
function initGrid() {
  ...
  let resizeMockRo = /** @type {ResizeObserver | null} */ (null);
  let resizeMockRaf = 0;
  /** @type {HTMLElement | null} */
  ...
  grid.on('resizestop', (_ev, el) => {
    ...
    fillEditorMocks(el instanceof HTMLElement ? el : undefined);
  });
}
...
function init() {
  ...
  loadItems(
    /** @type {AnbChartItem[]} */ (
      /** @type {{ items: AnbChartItem[] }} */ (state.base.chart).items
    ),
  );
  setActiveMessenger('telegram');
  scheduleFitEditorScale();
