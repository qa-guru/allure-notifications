...
function kitOnlyPaletteBodyHtml(panelId, chartType) {
 ...
 */
function kitOnlyQgMockHtml(panelId, chrome, barTrailingHtml = '') {
    const testId = `anb-kit-mock-${panelId}`;
...
function previewItemHtml(item) {
    const chart = /** @type {{ width: number, height: number, headerHeight?: number, cardGap?: number, tilePad?: number }} */ (state.base.chart);
...
function panelInnerHtml(item) {
    ...
    if (item.groupBy)
        ...
        attrs += ` data-group-by="${escapeHtml(meta.groupBy)}"`;
    if (item.by)
        attrs += ` data-by="${escapeHtml(item.by)}"`;
    else if (meta?.by)
        attrs += ` data-by="${escapeHtml(meta.by)}"`;
    const deleteBtn = `<button type="button" class="anb-panel__action" data-anb-action="delete" title="Delete" aria-label="Delete">` +
    ...
    if (chartType === 'qualityGate' && panelId) {
        ...
        return (`<div class="anb-panel__body anb-panel__body--qg-hybrid">` +
