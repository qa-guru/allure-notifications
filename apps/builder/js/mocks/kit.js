import { GRID_COLS, isKitOnlyPanelId, isKitOnlyPanelType, resolvePanelMeta, } from '@qa-guru/allure-notifications-config';
...
export function syncCanvasTestsTables(root) {
 ...
 */
export function kitOnlyPanelMockHtml(panelId, chartType, qgChrome = 'hybrid', barTrailingHtml = '') {
    const testId = `anb-kit-mock-${panelId}`;
    if (chartType === 'testsTable') {
 ...
 * @returns {string}
 */
export function paletteItemHtml(item) {
 ...
 * @param {ChartItem} item
 */
export function panelInnerHtml(item) {
 ...
 * @param {ChartItem} item
 */
export function previewItemHtml(item) {
 ...
 */
export function fillEditorMocks(root) {
