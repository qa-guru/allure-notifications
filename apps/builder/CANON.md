...
**Accepted visual:** one hybrid bar + body. Collage/editor/TG bar = **title only** — **no** status indicator, **no** `(i)`/qg-info. Delete overlay on editor. Palette add = **2×2** (same as all panels). SQG compact `cov|72<80` + `bugs|3>0`, bar height = `--anb-bar-h`. Debug: [`debug-qg.html`](./debug-qg.html). Goldens: [`fixtures/qg-locked/`](./fixtures/qg-locked/). Guard: smoke `chart.profile kit`.

| Surface | Chrome | Forbidden |
|---------|--------|-----------|
| Jar collage PNG | Hybrid PNG full cell; title-only bar | macOS `drawCard`; indicator; info glyph |
| Editor canvas | Hybrid full-bleed; title-only; delete overlay; **2×2** add | `anb-panel__bar`; indicator; `(i)`; raw `31px` bar; non-2×2 default footprint |
